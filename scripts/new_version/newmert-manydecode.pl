...
# we run moses to check validity of moses.ini 
# and to obtain all the feature names
...
while(1) {
  ...
  my @MIN = @{$featlist->{"mins"}};   # lower bounds
  my @MAX = @{$featlist->{"maxs"}};   # upper bounds
  my @CURR = @{$featlist->{"values"}};  # the starting values
  my @NAME = @{$featlist->{"names"}};  # to which model does the lambda belong

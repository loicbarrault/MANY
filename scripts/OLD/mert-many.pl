...
# $Id$
# Usage:
# mert-many.pl <foreign> <english> <decoder-executable> <decoder-config>
...
# for each TERp parameter (_del_etion, _ins_ertion, _sub_stitution, _shift_, _stem_, _syn_onym, _match_) and each decoder parameter (_lm_weight, _null_penalty and _word_penalty),
# there is a list of [ default value, lower bound, upper bound ]-triples. 
# In most cases, only one triple is used, but the translation model has currently 5 features
...
my $default_triples = {
    ...
    "ins"  => [ [ 1.0, 0.0, 2.0 ] ],  # insertion cost 
    "del"  => [ [ 1.0, 0.0, 2.0 ] ],  # deletion cost 
    "sub"  => [ [ 1.0, 0.0, 2.0 ] ],  # substitution cost 
    "shift"  => [ [ 1.0, 0.0, 2.0 ] ],  # shift cost 
    "stem"  => [ [ 1.0, 0.0, 2.0 ] ],  # stem cost 
    "syn"  => [ [ 1.0, 0.0, 2.0 ] ],  # synonym cost 
    "match"  => [ [ 0.0, 0.0, 2.0 ] ],  # match cost 
...
# uses ABBR names.
my $ABBR_FULL_MAP = "ins=insertion del=deletion sub=substitution shift=shift stem=stem syn=synonym match=match lm=lm-weight word=word-penalty null=null-penalty priors=priors";
...
{
	push (@$arrayref, [ $def_prob, 0.0, 1.0 ]);    # prior probabilities
}

foreach my $k (sort keys %{$default_triples})
{
	#print "$k\n";
	#print "size : ";
	#print scalar @{$default_triples->{$k}};
	#print "\n";
	#print "@{$$default_triples{$k}->[0]}\n";
	
	for(my $kkk=0; $kkk<scalar @{$default_triples->{$k}}; $kkk++)
	{
	   print " default_triples{$k}->[$kkk] : @{$$default_triples{$k}->[$kkk]}  \n";
...
$many_cmd = "$MANY_ROOTDIR/MANY_template.pl" if !defined $many_cmd;
...
#print "BEFORE SCAN_CONFIG ...";
#foreach my $t (keys %used_triples){
#	print "used_triples{$t} : ";
#	for my $el (@{$used_triples{$t}}){
#		print "@{$el}";
#	}	
#	print "\n";
#}
...
while(1) {
  ...
  # run MANY with option to output nbestlists
  # the end result should be (1) @NBEST_LIST, a list of lists; (2) @SCORE, a list of lists of lists

  print "run $run start at ".`date`;

  # In case something dies later, we might wish to have a copy
  # create_config($___MANY_CONFIG, "./run$run.many.config.xml", \%used_triples, $run, (defined$devbleu?$devbleu:"--not-estimated--"));
	safesystem("\\cp -f $___MANY_CONFIG run$run.many.config.xml") or die "Cannot create a backup of the config file";
  ...
  foreach my $name (@order_of_lambdas_from_decoder) {
      ...
      print STDERR "order of lambda for $name ($visited{$name}) : ";
...
#create_config($___MANY_CONFIG_BAK, "./many.config.xml", \%used_triples, $run, $devbleu);
safesystem("\\cp -f run$run.many.config.xml $___MANY_CONFIG") or die "Cannot restore config file";
...
sub run_many {
    ...
    # run MANY
	my $nBest_cmd = "-nbest-size $___N_BEST_LIST_SIZE";
    my $many_cmd = "$___MANY ";
	...
	if(defined $___NB_THREADS){
		$many_cmd .= " $many_config $parameters --nbest-file $filename --nbest-size $___N_BEST_LIST_SIZE --multithread $___NB_THREADS > run$run.out";
	}
	else {
		$many_cmd .= " $many_config $parameters --nbest-file $filename --nbest-size $___N_BEST_LIST_SIZE > run$run.out";
	}
...
sub scan_many_config {
  ...
  while (<XML>) {
    ...
    if(/^<property name=\"(.+)\"\s+value=\"(.+)\"/) {
        ...
        my $weightval = $2;
        my @tab;
        #print "LAMBDA : $weightname : $weightval\n";
        if( defined($FULL2ABBR{$weightname}) ){
            ...
            push @{$config_weights->{$weightname}}, @tab;
        }
    }
  }

...
public class TERWordClassCost extends TERcost
{
	...
	TERWordClassCost(String fname, double dsub_cost, double dmat_cost, double dins_cost, double ddel_cost,
			double dsht_cost, double dstm_cost, double dsyn_cost)
	{
	...
	private static final Pattern cost_p1 = Pattern.compile("^([IiDdMmTt])\\s+(.*?)\\s+([0-9.]+)\\s*$");
	private static final Pattern cost_p2 = Pattern.compile("^([Ss])\\s+(.*?)\\s+(.*?)\\s+([0-9.]+)\\s*$");
	...
	public static String NUMBER_STR()
	{
		return NUMBER_STR;
	}
	...
	public void get_wgt_edit(double[] wgt, char type, Comparable hyp, Comparable ref)
	{
		switch (type)
		{
			case ' ' :
				...
				break;
			case 'I' :
				wgt[(1 * numClasses) + findClass(hyp)]++;
				break;
			case 'D' :
				wgt[(2 * numClasses) + findClass(ref)]++;
				break;
			case 'S' :
				wgt[(4 * numClasses) + (findClass(ref) * numClasses) + findClass(hyp)]++;
				break;
			case 'T' :
				wgt[(4 * numClasses) + (numClasses * numClasses) + (findClass(ref) * numClasses) + findClass(hyp)]++;
				break;
			case 'Y' :
				wgt[(4 * numClasses) + (2 * (numClasses * numClasses)) + (findClass(ref) * numClasses) + findClass(hyp)]++;
				break;
		}
		return;
	}
	...
	public double[] current_weights()
	{
		...
		for (int i = 0; i < numClasses; i++)
		{
			...
			for (int j = 0; j < numClasses; j++)
			{
				...
				tr[(4 * numClasses) + (numClasses * numClasses) + (numClasses * i) + j] = _stem_cost[i][j];
				tr[(4 * numClasses) + (2 * numClasses * numClasses) + (numClasses * i) + j] = _syn_cost[i][j];
			}
		}
		int my_num_feat = (4 * numClasses) + (3 * numClasses * numClasses);
		if (_phraseTable != null)
	...
	public void set_weights(double[] wgts)
	{
		...
		for (int i = 0; i < numClasses; i++)
		{
			...
			for (int j = 0; j < numClasses; j++)
			{
				...
				_stem_cost[i][j] = wgts[(4 * numClasses) + (numClasses * numClasses) + (i * numClasses) + j];
				_syn_cost[i][j] = wgts[(4 * numClasses) + (2 * numClasses * numClasses) + (i * numClasses) + j];
			}
		}
		if (_phraseTable == null)
			return;
	...
	public void get_wgt_phrase(double[] wgt, Comparable[] hyp, int hstart, int hlen, Comparable[] ref, int rstart,
			int rlen)
	{
		if (_phraseTable == null)
			return;
	...
	public void load_word_classes(String fname)
	{
		...
		{
			Map.Entry e = (Map.Entry) it.next();
			for (Iterator it2 = ((HashMap) e.getValue()).entrySet().iterator(); it2.hasNext();)
			{
				Map.Entry e2 = (Map.Entry) it2.next();
				_sub_cost[((Integer) e.getKey()).intValue()][((Integer) e2.getKey()).intValue()] = ((Double) e2
		...
		for (Iterator it = syn_cost.entrySet().iterator(); it.hasNext();)
		{
			...
			{
				Map.Entry e2 = (Map.Entry) it2.next();
				_syn_cost[((Integer) e.getKey()).intValue()][((Integer) e2.getKey()).intValue()] = ((Double) e2
						.getValue()).doubleValue();
			}
		}
		for (Iterator it = stem_cost.entrySet().iterator(); it.hasNext();)
		{
			...
			{
				Map.Entry e2 = (Map.Entry) it2.next();
				_stem_cost[((Integer) e.getKey()).intValue()][((Integer) e2.getKey()).intValue()] = ((Double) e2
						.getValue()).doubleValue();
			}
		}
	}
	...
	/*
	 * The cost of substituting ref word for hyp word. (They are not equal, but
	 * have the same stem)
	 */
	...
	{
		word_class[] wc = new word_class[in.length];
		for (int i = 0; i < in.length; i++)
		{
			wc[i] = new word_class(in[i], _findClass(in[i]));
		}
		return (Comparable[]) wc;
	}
